...
if [ -f "$ORGS_DIR/ordererOrganizations/example.com/orderers/orderer.example.com/tls/server.crt" ] && \
   [ -f "$ORGS_DIR/peerOrganizations/org1.example.com/peers/peer0.org1.example.com/tls/server.crt" ]; then
    echo "✅ Target crypto certificates already exist. Skipping generation."
else
    echo "⚠️ Target crypto certificates missing or incomplete. Cleaning up stale directories..."
    rm -rf "$ORGS_DIR/ordererOrganizations" "$ORGS_DIR/peerOrganizations" "$ORGS_DIR/channel-artifacts"
    
    echo "📝 Generating crypto config using cryptogen..."
    cryptogen generate \
...
echo "✅ Crypto material and genesis block generated successfully!"
