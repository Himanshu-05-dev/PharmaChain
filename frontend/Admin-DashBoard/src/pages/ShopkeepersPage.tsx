      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">
            Verify Form 20/21 retail & Form 20B/21B wholesale certificates before granting dispensary access.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-3">
        ...
        <div className="flex items-center gap-1.5 bg-slate-100/90 dark:bg-slate-800/80 p-1.5 rounded-2xl border border-slate-200/80 dark:border-slate-700/60 shadow-inner">
          ...
          >
            ...
            <span className={`px-2 py-0.5 rounded-full text-[10px] font-black ${activeTab === 'pending' ? 'bg-blue-100 dark:bg-blue-950 text-blue-800 dark:text-blue-300' : 'bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300'}`}>
              {pendingCount}
            </span>
          </button>
          ...
          >
            <span>Suspended</span>
            <span className={`px-2 py-0.5 rounded-full text-[10px] font-black ${activeTab === 'suspended' ? 'bg-red-200 dark:bg-red-950 text-red-900 dark:text-red-300' : 'bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300'}`}>
              {suspendedCount}
            </span>
          </button>
        </div>
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <select
            ...
            className="text-xs rounded-2xl border border-slate-200 dark:border-slate-700/80 bg-white/90 dark:bg-slate-800/90 px-3.5 py-2.5 text-slate-700 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm"
          >
              ...
              {filteredData.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-16 text-center text-slate-400 dark:text-slate-500">
                    ...
                    <p className="text-sm font-semibold text-slate-600 dark:text-slate-400">No pharmacy records matching filter</p>
                ...
                filteredData.map((shop) => {
                  ...
                  return (
                    <tr key={shop.shopId} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors group">
                      {/* Pharmacy & Location */}
                      <td className="px-6 py-4.5">
                        <div className="font-bold text-slate-900 dark:text-white text-xs">{shop.shopName}</div>
                      ...
                      {/* Owner / Pharmacist */}
                      <td className="px-6 py-4.5">
                        <div className="font-semibold text-slate-800 dark:text-slate-200">{shop.ownerName}</div>
                        <div className="text-[11px] text-slate-500 dark:text-slate-400 font-mono">{shop.ownerPhone}</div>
                      </td>

                      {/* License Info */}
                      <td className="px-6 py-4.5">
                        <div className="font-mono font-bold text-slate-800 dark:text-slate-200 text-xs">{shop.drugLicenseNumber}</div>
                        <div className="flex items-center gap-2 mt-1">
                          <Badge variant={shop.licenseType} size="sm" />
                          <span className={`text-[10px] font-semibold ${isExpired ? 'text-rose-600 dark:text-rose-400' : 'text-slate-500 dark:text-slate-400'}`}>
                      ...
                      <td className="px-6 py-4.5">
                        {shop.documentUrl ? (
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 text-xs text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-950/60 border border-blue-200 dark:border-blue-800 px-3 py-1.5 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/60 transition-all shadow-sm"
                          >
                            <FileText className="w-3.5 h-3.5" />
                            <span className="font-medium">Inspect Certificate</span>
                          </button>
                        ) : (
